...
export function CommissionerReportTab({
  ...
  const fetchReport = useCallback(
    async (week?: string) => {
      ...
      try {
        ...
        if (!week && res.available_weeks.length > 0) {
          setSelectedWeek(res.available_weeks[res.available_weeks.length - 1])
        }
      } catch {
        /* ignore */
      } finally {
  ...
  async function handleGenerate() {
    ...
    setMsg(null)
    try {
      const updated = await generateCommissionerReport(token, id, selectedWeek)
      setReport(updated)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Empty state */}
      {!report && !editing && availableWeeks.length === 0 && (
        <div className="text-center py-12">
          <svg className="mx-auto w-12 h-12 text-slate-700 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
          </svg>
          <p className="text-slate-500 text-sm">
            No scoring weeks recorded yet. Commissioner reports will be available once the season starts.
          </p>
        </div>
      )}
